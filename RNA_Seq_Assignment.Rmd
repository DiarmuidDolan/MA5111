...
### Checking Numerics to convert to Factors
...
Tximport imports transcript-level abundances from quantification tools such as kallisto and converts them to gene counts for downstream analysis.
...
The model design is the most important element of our RNA Seq Analysis, here we will specify the design model.
...
This code gets important gene data. It adjusts for differences in measurements. Storing the data as 'counts' and is then ready for detailed analysis. This is key for understanding gene behaviour.
...
For example, the first row has sample numbers (e.g., [,1], [,2], ..., [,4]). Above are genes (e.g., A1BG, A1CF, A2M, A2ML1), with numbers showing their levels. This data helps study how genes react in experiments.
...
The 'meanSdPlot' function generates a plot displaying the relationship between the mean and standard deviation of the data. This helps in understanding the data's distribution and variability. We can see as the mean increases the relationship between the mean and sd decreases with the majority of the counts being towards the left hand side of the plot and they steadily decrease as the mean increases, implying that there are maybe more downregulated genes present in the counts. 
...
This is typical of count data as most genes will be expressed at moderate levels.
...
This distance matrix is a crucial tool for understanding the similarity or dissimilarity between samples based on their gene expression profiles. It can be used for various types of analysis, such as clustering or multidimensional scaling, to explore patterns or relationships within the dataset.
...
PCA plots the variance explained by samples in each principal component. Typically PC1 & PC2 explain the most variation in a dataset, so they are the two we will be focusing on.
...
It is quite hard to interpret due to the amount of genes present so I provided a heat map of the top ten upregulated genes, within it the same type of colour pattern can be seen, just much clearer.
...
This table displays HGNC symbols (gene names) alongside their corresponding average log2 fold changes. Each row represents a unique gene, providing valuable insights into the differential expression patterns observed in the dataset. These values serve as a crucial foundation for downstream analysis, such as gene set enrichment analysis (FGSEA), aiding in the identification of biologically relevant pathways and processes.
...
The first plot visualises the enrichment scores for the pathway "CAHOY_NEURONAL". The x-axis represents the ranking of genes, while the y-axis shows the enrichment score. The green line tracks the enrichment scores, with numerous black lines marking the ranks/genes. The green line rises steeply towards an enrichment score of around 0.15, around rank 2200, and descends, hitting the bottom red dotted line at roughly -0.4 at around rank 17500. This suggests a significant downregulation pattern in this pathway.

The second plot visualises the enrichment scores for the pathway "CSR_LATE_UP.V1_DN". The x-axis represents the ranking of genes, while the y-axis shows the enrichment score. The green line tracks the enrichment scores, with numerous black lines marking the ranks/genes. The green line rises steeply once again, hitting the top red dotted line at an enrichment score of roughly 0.1 around rank 2200, and descends, hitting the bottom red dotted line at roughly -0.4 at around rank 17500. This suggests a significant downregulation pattern in this pathway.

The third plot visualises the enrichment scores for the pathway "MTOR_UP.V1_UP". The x-axis represents the ranking of genes, while the y-axis shows the enrichment score. The green line tracks the enrichment scores, with numerous black lines marking the ranks/genes. The green line falls/maintains for the first bit and hits just above 0.0 at around 2500 then falls quite steeply to around 17500 and at that point it has an enrichment score of just under -0.4. This suggests a significant downregulation pattern in this pathway.
